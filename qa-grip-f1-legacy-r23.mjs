...
// Human-accepted R13 region and R22.1 mapping must remain untouched by cleanup.
...
assert.match(runtime,/\?blendDriftForce\([\s\S]*legacyForceTrajectoryYawRate,[\s\S]*physicalTrajectoryYawRate,[\s\S]*driftPhysicalAuthority[\s\S]*\)[\s\S]*:physicalTrajectoryYawRate;/,'legacy trajectory blend is not bypassed for physical-only profiles');
...
// R23 also guarantees that normal longitudinal tire load still participates in
// the friction circle even though the old extra RWD grip-loss heuristic is off.
const contacts=[
