...
for(const name of [...names].sort()){
  ...
  const re=new RegExp(`(?:globalThis|window)\\.${esc(name)}\\s*=`);
  const writers=sourceRefs.filter(r=>re.test(r.text));
...
console.log('CLEANUP C6.5 REMAINING DIAGNOSTIC GLOBAL AUDIT: PASS');
