...
assert.ok(
  source.includes("const runningIntensity=night>.06 ? (.90+night*3.60) : .02;"),
...
const fullNight=.90+1*3.60;
assert.equal(fullNight,4.5,'WRX full-night running-tail intensity must be 4.5');
assert.ok(fullNight<5.0,'night running lamps must remain below brake intensity');

console.log('PASS WRX rear night running lights');
console.log('  - full-night running-tail intensity: 4.5');
