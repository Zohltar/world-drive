...
assert.ok(source.includes("getObjectByName('Object_7')"),'Traffic R4 must reuse the authored front Sonata lens mesh');
assert.ok(source.includes("getObjectByName('Object_46')"),'Traffic R4 must reuse the authored inner rear Sonata lens mesh');
assert.ok(source.includes("getObjectByName('Object_33')"),'Traffic R4 must reuse the authored outer rear Sonata lens mesh');
assert.ok(source.includes('new THREE.ShaderMaterial('),'Traffic R4 lamp glow must be texture-filtered on authored meshes');
assert.ok(source.includes('texture2D(uMap,vUv)'),'Traffic R4 glow must sample the original authored lamp texture');
assert.ok(!source.includes('new THREE.SphereGeometry(.085'),'Traffic R4 must not reintroduce generic visible lamp bulbs');
assert.ok(!source.includes('new THREE.SpotLight('),'Traffic R4 must not project white headlight pools onto the road');
assert.ok(!source.includes('new THREE.PointLight('),'Traffic R4 must not project red/white spill onto the road');
assert.ok(source.includes("mode:'traffic-r4-authored-lamps-no-road-spill'"),'Traffic R4 diagnostics mode must identify authored-only road-spill-free lighting');
assert.ok(source.includes('roadLightSpill:false'),'Traffic R4 diagnostics must report road-light spill disabled');

console.log('PASS Traffic R4 sparse civil traffic policy');
...
console.log('  - no PointLight/SpotLight road spill remains');
