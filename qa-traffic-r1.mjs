...
assert.ok(source.includes("getObjectByName('Object_7')"),'Traffic R5 must reuse the authored front Sonata lens mesh');
assert.ok(source.includes("getObjectByName('Object_46')"),'Traffic R5 must reuse the authored inner rear Sonata lens mesh');
assert.ok(source.includes("getObjectByName('Object_33')"),'Traffic R5 must reuse the authored outer rear Sonata lens mesh');
assert.ok(source.includes('new THREE.ShaderMaterial('),'Traffic R5 lamp glow must be texture-filtered on authored meshes');
assert.ok(source.includes('texture2D(uMap,vUv)'),'Traffic R5 glow must sample the original authored lamp texture');
assert.ok(!source.includes('new THREE.SphereGeometry(.085'),'Traffic R5 must not reintroduce generic visible lamp bulbs');
assert.ok(source.includes('new THREE.SpotLight(0xf8fbff,0,72,.36,.68,1.0)'),'Traffic R5 must use the pilotable Sonata SpotLight contract');
assert.ok(source.includes('target.position.set(side*.45,.15,30);'),'Traffic R5 headlight target must match the pilotable Sonata');
assert.ok(source.includes('beam.position.set(side*.68,.66,2.25);'),'Traffic R5 beam position must match the pilotable Sonata');
assert.ok(source.includes('beam.light.intensity=nightOn?night*95:0;'),'Traffic R5 beam intensity must match the pilotable Sonata');
assert.ok(source.includes('beam.light.distance=65+night*15;'),'Traffic R5 beam distance must match the pilotable Sonata');
assert.ok(!source.includes('new THREE.PointLight('),'Traffic R5 must not reintroduce point-light pools');
assert.ok(source.includes("mode:'traffic-r5-player-sonata-headlights'"),'Traffic R5 diagnostics mode must identify pilotable-Sonata headlights');
assert.ok(source.includes('rearRoadLightSpill:false'),'Traffic R5 diagnostics must report rear road spill disabled');

console.log('PASS Traffic R5 sparse civil traffic policy');
...
console.log('  - forward headlight beams match the pilotable Sonata');
