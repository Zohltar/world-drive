...
const terrainPath=path.join(root,'src','terrain.js');
const source=fs.readFileSync(terrainPath,'utf8').replace(/\r\n/g,'\n');
...
for(const pattern of [
  ...
  /const naturalCopyFilter=filterNaturalCopyTransitionTriangles\(group/,
  /naturalCopyTrianglesRemoved:naturalCopyFilter\.trianglesRemoved/
])assert.match(source,pattern,`Issue 4 natural-copy guard missing: ${pattern}`);

const moduleUrl=`${pathToFileURL(terrainPath).href}?qa=${Date.now()}`;
...
console.log({
  ...
  syncAndIncrementalCovered:true
});
