...
pilot.start({directory:identity,baseUrl:'http://example.invalid/',presentation:'mixed-r16',appearance:'natural-r17'});
...
check('one source proof produces both approved model families without moving trees',()=>{
  const d=pilot.audit().meshes[0];assert.equal(pilot.diagnostics().pilot,'r17-nord-natural');assert.equal(d.proofCandidates,1744);
