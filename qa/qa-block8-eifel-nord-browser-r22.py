    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                ref=page.evaluate("async u=>(await import(u)).start({season:'summer'})",REFERENCE);assert ref['pilot']=='r18-nord-understory';page.wait_for_function("()=>{const s=WorldDriveDiagnostics.forest.visualPilot.snapshot();if(s.phase==='fault')throw new Error(s.error);return s.modifiedChunks>=3&&s.proofsCompleted>=3;}")
                ...
                page.evaluate("()=>WorldDriveDiagnostics.forest.visualPilot.season('summer')");state=page.evaluate(SNAP);page.evaluate("()=>{const i=document.getElementById('jump');i.value=50;i.dispatchEvent(new Event('input'));document.getElementById('jumpBtn').click();WorldDriveDiagnostics.forest.visualPilot.refresh();}");page.wait_for_timeout(3000);after=page.evaluate(SNAP);after_audit=page.evaluate('()=>WorldDriveDiagnostics.forest.visualPilot.audit()');report['afterJump']={'snapshot':after,'audit':after_audit};assert after['enabled'] and after['presentation']=='eifel-temperate-r22' and after['error'] is None and all(m.get('mixed',{}).get('sourcePrefixExact') for m in after_audit['meshes']);page.screenshot(path=str(output/'nord-r22-after-jump.png'))
