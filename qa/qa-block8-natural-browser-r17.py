...
LAUNCHER='/tools/biomes/natural-pilot-launcher-r17.mjs'
...
def main(pilot,output):
    ...
    if destination.exists():raise FileExistsError('Do not overwrite installed R17 data')
    ...
    try:
      ...
      with sync_playwright() as pw:
        ...
        def network(route):
            ...
            return route.fulfill(status=503,content_type='text/plain',headers=headers,body='R17 upstream fixture')
        ...
        page.screenshot(path=str(output/'nord-natural-summer.png'))
        report['seasonIdentity']=page.evaluate(COMPARE)
        report['winterFrames']=page.evaluate(R9.FRAMES,3000)
        report['winter']=page.evaluate(SNAP);page.screenshot(path=str(output/'nord-natural-winter.png'))
        ...
        report.update(pageErrors=errors,engineErrors=engine,pilotRequests=requests,upstreamRequests=dict(upstream),
            ...
            'Mixture only inside fully source-qualified Nordschleife chunks; 3:1 artistic weight, not local species composition','Parked sampling and an actual UI teleport, not continuous high-speed driving','Two visible mesh passes per proved chunk; new alpha-tested foliage raises texture/fragment and triangle cost, not zero GPU cost', 'Only summer appearance is reauthored; approved winter shapes are retained. Reference photograph is art direction, not copied texture.'])
        (output/'rendered-pilot-r17-qa.json').write_text(json.dumps(report,indent=2)+'\n')
        server.terminate()
