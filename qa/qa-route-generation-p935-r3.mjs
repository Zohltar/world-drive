...
const lifecycle=createRouteLifecycle({
  version:'route-generation-r5-p935',
  getState:()=>state,
...
assert.equal(switches.length,1,'initial A did not claim the scenery renderer forest slot');
...
assert.equal(switches.length,2,'B did not switch to its scenery renderer forest slot before P9.35');
...
assert.equal(switches.length,3,'return A did not switch scenery renderer forest ownership');
...
console.log('POST-REFACTOR ROUTE GENERATION P9.35 R5 QA: PASS',{
  owner:'sceneryRenderer.switchForestRouteCache',
