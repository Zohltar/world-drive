...
check('humid montane assets expose Yungas broadleaf epiphyte tree-fern and bamboo cues with one material',()=>{
  ...
  assert.ok(a[2].geometry.boundingBox.max.y>=1.03);assert.ok(d.treeFernSilhouette.height>=1.03);assert.ok(d.treeFernSilhouette.diameterX>=1.9&&d.treeFernSilhouette.diameterZ>=1.9);assert.equal(d.treeFernSilhouette.fronds,20);assert.equal(d.treeFernSilhouette.segments,9);assert.equal(d.treeFernSilhouette.pinnae,320);assert.ok(d.treeFernSilhouette.pinnaNearCrownFullWidth>=.24);assert.ok(d.treeFernSilhouette.pinnaOuterFullWidth>=.21);assert.ok(a[3].geometry.boundingBox.max.y>.75);assert.match(d.scope,/prominent tree ferns/);assert.equal(d.mix.treeFernVisualWeight,20);assert.equal(d.mix.measuredHabitatPercent,false);assert.equal(d.mix.altitudeZonation,false);
});
