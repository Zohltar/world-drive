...
import {R12_PROFILE,R15_PROFILE,R12_REGION,R15_REGION,R12_SOURCE,R12_CATALOG_SHA,R12_LIMITS,
...
const coordinates=JSON.parse(gunzipSync(fs.readFileSync(new URL('fixtures/biomes/yungas-r15/response.json.gz',import.meta.url)))).routes[0].geometry.coordinates;
const origin={lon:-67.81891,lat:-16.29911},route=coordinates.map(([lon,lat])=>({lon,lat}));
const plan=createBiomeObserverPlan(route,{origin,routeId:'visual-1'}),adapter=plan.adapter;
const identity={revision:renderedProfile(R15_PROFILE).revision,source:R12_SOURCE,catalogSha256:R12_CATALOG_SHA};
const context=Object.freeze({schema:'world-drive-biome-context-v1',...BIOME_PROFILES[1],status:'resolved',reason:null,
  source:R12_SOURCE,ecoregion:R15_REGION,precision:'source-polygons',confidence:'source-agreement',
  paletteEligible:true,placementAuthority:false,elevationApplied:false});
const config={profile:R15_PROFILE,directory:identity,baseUrl:'http://example.invalid/data/',season:'summer'};
...
function proof(s=snap()){return createR12Proof(s,adapter,s.cx,s.cz,R15_PROFILE);}
...
await test('fixed profile/model IDs preserve Nord defaults and reject unapproved tropical winter',()=>{
  assert.equal(r12Model('summer'),'preview-temperate');assert.equal(r12Model('winter'),'preview-temperate-winter');
  assert.equal(r12Model('summer',R15_PROFILE),'preview-tropical');assert.throws(()=>r12Model('winter',R15_PROFILE),/non disponible/);
  assert.ok(Object.isFrozen(renderedProfile(R15_PROFILE)));assert.ok(Object.isFrozen(R15_REGION));
...
await test('configuration binds profile to revision and the original pinned source',()=>{
  assert.equal(validateR12Config(config).profile,R15_PROFILE);
...
await test('regional eligibility rejects guesses, other realms/families and placement authority',()=>{
  assert.equal(r12ContextEligible(context,R15_PROFILE),true);assert.equal(r12ContextEligible(context),false);
  for(const patch of [{ecoregion:R12_REGION},{family:'temperate'},{palette:'temperate-conifer'},
    ...
    {ecoregion:{...R15_REGION,realm:'Palearctic'}},{ecoregion:{...R15_REGION,name:'Guess'}},
    {source:{...R12_SOURCE,license:'unknown'}}])assert.equal(r12ContextEligible({...context,...patch},R15_PROFILE),false);
});
await test('all 1744 exact addresses are read with one immutable eligibility decision',()=>{
  ...
  const r=finish(p);assert.equal(r.ecoregionId,444);assert.equal(r.modelId,'preview-tropical');assert.equal(r.profileId,R15_PROFILE);
...
await test('one missing, mixed or mutable-last source context refuses the whole chunk',()=>{
  ...
  const region={...R15_REGION},mutable=Object.freeze({...context,ecoregion:region});
...
await test('Yungas admission allows resampling but binds endpoints, waypoint, length and envelope',()=>{
  assert.equal(renderedRouteMatches(R15_PROFILE,plan,origin),true);
  assert.equal(renderedRouteMatches(R12_PROFILE,plan,origin),false);
  const changed=coordinates.filter((p,i)=>i%2===0||i===coordinates.length-1);
  assert.equal(renderedRouteMatches(R15_PROFILE,{...plan,coordinates:changed},origin),true);
  for(const modified of [[...coordinates].reverse(),coordinates.map((p,i)=>i===100?[0,p[1]]:p),
    coordinates.map((p,i)=>i===100?[true,p[1]]:p),coordinates.map((p,i)=>i===100?[p[0],NaN]:p),
    coordinates.filter(p=>Math.abs(p[0]+67.7861)>.0045||Math.abs(p[1]+16.2577)>.0045),[]]){
    assert.equal(renderedRouteMatches(R15_PROFILE,{...plan,coordinates:modified},origin),false);
  }
  for(const totalMeters of [0,9999,45001,Infinity,NaN,'28000',true])assert.equal(renderedRouteMatches(R15_PROFILE,{...plan,totalMeters},origin),false);
  assert.equal(renderedRouteMatches(R15_PROFILE,plan,{...origin,lat:0}),false);
  assert.equal(renderedRouteMatches(R15_PROFILE,plan,null),false);
});
class Group{
...
function setup({bad=false,wrong=false,clock=()=>0}={}){
  const windows=[],q=[],base=geo(),summer=geo(),root=new Group('forest'),owner=new Group('forest-route-cache-yungas');root.add(owner);
  ...
  const pilot=createRenderedBiomePilot({THREE:{},forestGroup:root,getState:()=>state,getGeneration:()=>generation,
    ...
    assetFactory:()=>{builds++;return {summerAssets:[{parts:[{geometry:base}]}],assets:[
      {id:'preview-tropical',parts:[{geometry:summer}]}],dispose(){}};},
...
await test('R15 remains fully inert before explicit activation',()=>{
...
await test('tropical geometry-only replacement preserves all original instance and scene state',async()=>{
  ...
  assert.equal(h.pilot.diagnostics().pilot,R15_PROFILE);assert.equal(m.geometry,h.summer);assert.equal(m.count,100);
  ...
  assert.equal(h.pilot.audit().meshes[0].ecoregion,444);h.pilot.stop();assert.equal(m.geometry,h.base);assert.equal(h.counts().disposed,1);
});
...
await test('new R4 layer inherits tropical proof, while hidden same-address route remains untouched',async()=>{
...
await test('invalid profile or revision does not destroy a working tropical presentation',async()=>{
  const h=setup();h.pilot.start(config);await h.until(()=>h.pilot.diagnostics().modifiedChunks===1);
...
await test('tropical proof keeps per-read time budget and never executes on denied idle slots',async()=>{
...
await test('finite circuit window remains bounded in both directions and preserves Nord options',async()=>{
  ...
  assert.deepEqual(renderedWindowOptions(R15_PROFILE,1),options);
  assert.deepEqual(renderedWindowOptions(R15_PROFILE,-1),options);
  assert.deepEqual(renderedWindowOptions(),options);
  assert.ok(Object.isFrozen(renderedWindowOptions(R15_PROFILE)));assert.throws(()=>renderedWindowOptions(R15_PROFILE,0));
  const h=setup();h.pilot.start(config);await h.until(()=>h.pilot.diagnostics().modifiedChunks===1);
  assert.deepEqual(h.windows[0].options,options);h.pilot.stop();
});
await test('finite multi-chunk tropical decisions remain deterministic',()=>{
  for(let n=0;n<128;n++){const r=finish(proof(snap(-n,n)));assert.equal(r.count,1744);assert.equal(r.ecoregionId,444);assert.equal(r.modelId,'preview-tropical');}
});
