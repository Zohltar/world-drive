...
const states={
  ...
  night:{gear:1,braking:false,reversing:false,nightLevel:.8,signalLeft:false,signalRight:false,signalBlink:false},
  brake:{gear:1,braking:true,reversing:false,nightLevel:.4,signalLeft:false,signalRight:false,signalBlink:false},
  'reverse-stopped':{gear:-1,speed:0,braking:false,reversing:false,nightLevel:0,signalLeft:false,signalRight:false,signalBlink:false},
  'reverse+brake+night':{gear:-1,speed:0,braking:true,reversing:false,nightLevel:1,signalLeft:false,signalRight:false,signalBlink:false},
  'left-on':{gear:1,signalLeft:true,signalRight:false,signalBlink:true},
  'left-off':{gear:1,signalLeft:true,signalRight:false,signalBlink:false},
  'right-on':{gear:1,signalLeft:false,signalRight:true,signalBlink:true}
};
...
assert.equal(normalized['reverse-stopped'].gear,-1);
assert.equal(normalized['reverse-stopped'].reversing,true,'gear R must force reverse independently of vehicle speed or legacy bool');
...
assert(client.includes('const reversing=reverseFromGear(gear'),'sender reverse must derive from gear');
assert(client.includes('const remoteReversing=reverseFromGear(peer.gear,peer.reversing)'),'receiver reverse must derive from replicated gear');
for(const field of ['gear:lighting.gear','braking:lighting.braking','reversing:lighting.reversing','nightLevel:lighting.nightLevel','signalLeft:lighting.signalLeft','signalRight:lighting.signalRight','signalBlink:lighting.signalBlink'])assert(client.includes(field),`network snapshot missing ${field}`);
...
console.log('V21.31 MULTIPLAYER M4.1 LIGHTING STATE MATRIX QA: PASS',{
  states:Object.keys(states),
  coverage,
  reverseSource:'network gear',
