#!/usr/bin/env python3
"""Real Vite/Three game and native Worker, finite Laguna woodland pilot.
...
R9=load('r14_r9',ROOT/'qa/qa-block8-biome-fullgame-r9.py')
NETWORK=load('r14_network',ROOT/'qa/qa-block8-rendered-manic-network-r13.py')
ORACLE=r'''async ({directory,base,plan,expected})=>{
 ...
 const {R14_PROFILE,renderedWindowOptions,createR12Proof}=await import('/tools/biomes/rendered-pilot-policy-r12.mjs');
 const a=createForestCandidateAdapter({origin:plan.origin,routeId:'r14-oracle'}),rp=createBiomeRoutePlan(plan.coordinates),client=createBiomeWorkerClient();
 ...
 try{
  ...
  for(const s of plan.samples){
   if((await bridge.update(rp.distanceAtVertex(s.anchorIndex),renderedWindowOptions(R14_PROFILE))).status!=='ready')throw new Error('oracle window');
   ...
   for(let i=0;i<1744;i++){
    ...
    regions[id]=(regions[id]??0)+1;all&&=id===423;ids.add(id);reads++;
   }
   const proof=createR12Proof(r.snapshot,a,s.cx,s.cz,R14_PROFILE);let step;
  ...
  if(reads!==83712||eligible!==39||refused!==9||mixed!==7)throw new Error('oracle coverage changed');
...
CHECK=r'''()=>{
 ...
 for(const m of before.meshes)if(m.model!=='preview-woodland'||m.triangles!==44||m.baseTriangles!==68||m.proofCandidates!==1744||m.ecoregion!==423||!m.geometryOnly)throw new Error('woodland geometry');
 let rejected=0;
 for(let i=0;i<20;i++){
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            def network(route):
                ...
                return route.fulfill(status=503,content_type='text/plain',headers=headers,body='R14 controlled geographic fixture')
            ...
            try:
                ...
                report['offBefore']=page.evaluate(R9.FRAMES,3000);page.screenshot(path=str(output/'laguna-off.png'))
                result=page.evaluate('async u=>(await import(u)).start()',URL+'start.mjs');assert result['pilot']=='r14-laguna-woodland'
                ...
                page.screenshot(path=str(output/'laguna-woodland.png'))
                ...
                for percent in [25,75]:
                    before_polls=page.evaluate(SNAP)['polls']
                    ...
                    page.wait_for_function("since=>{const s=WorldDriveDiagnostics.forest.visualPilot.snapshot();if(s.phase==='fault')throw new Error(s.error);return s.polls>=since+2 && s.modifiedChunks>=4;}",arg=before_polls)
                ...
                report['visibleAfterJumps']=jumps;page.screenshot(path=str(output/'laguna-after-jump.png'))
        ...
        report.update(pageErrors=errors,engineErrors=engine,pilotRequests=requests,upstreamRequests=dict(upstream),
            limitations=['Software rasterization, not user GPU or high-speed fluidity','External geographic services controlled; pale fallback ground is not snow',
                'Summer-only shape substitution; existing R4 density not ecological sparse placement','Mixed/non-423 chunks retain original forest; no source-boundary blending',
                'Parked samples and actual UI teleports, not a continuous driven lap'])
        (output/'rendered-laguna-r14-qa.json').write_text(json.dumps(report,indent=2)+'\n')
        if server:
            server.terminate()
