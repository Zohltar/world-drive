#!/usr/bin/env python3
"""R14 reproducible package and negative authoring inputs, no installed-data writes."""
import argparse
import gzip
import hashlib
...
spec=importlib.util.spec_from_file_location('r14_builder',ROOT/'tools/biomes/build-rendered-laguna-r14.py')
...
def main(source,atlas,pilot,output):
    ...
    assert manifest['tiles']==9 and manifest['batches']==1 and manifest['supportedSeasons']==['summer']
    assert manifest['oracleCandidates']==83712 and manifest['oracleChunks']==48
    for name,entry in manifest['files'].items():
        p=data/name;raw=p.read_bytes();assert len(raw)==entry['bytes'] and B.sha(raw)==entry['sha256']
        if name.endswith('.gz'):assert json.loads(gzip.decompress(raw))['sourceSha256']==B.sha(source.read_bytes())
    archive=pilot/'world-drive-biome-pilot-r14.zip'
    ...
    with tempfile.TemporaryDirectory(prefix='r14-builder-qa-') as tmp:
