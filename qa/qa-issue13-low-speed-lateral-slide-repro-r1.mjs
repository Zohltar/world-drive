...
function presentationProbe(vehicleId,pitchDeg,twist,{frames=260}={}){
  ...
  const presentation=createVehiclePresentation({
    THREE,scene,car:visuals.car,bodyGroup:visuals.bodyGroup,wheels:visuals.wheels,
    vehicleSystem,sun,
    roadSurfaceAt:(x,z)=>({lateral:0,y:groundHeight(x,z)}),
    terrainAbs:groundHeight,groundHeightForWheel:groundHeight,
  ...
  return {vehicle,contacts:copyContacts(presentation.wheelContacts),airborne:!!presentation.airborne};
}
...
function gripProbe(vehicle,contacts,{speed,steerInput,throttle}={}){
  ...
  const grip=estimateWheelGripUsage({
    ...
    surfaceMu:longitudinalMu,throttle,handbrake:false,airborne:false,vehicle,speedAbs:speed,dt:1/60,
    contacts,previousUsage:new Array(Math.max(4,contacts.length)).fill(0)
  },{});
  return {
    steerAngle,requestedLatAccel:Number(lat.requestedLatAccel)||0,latLimit:Number(lat.latLimit)||0,
    driveAccel:Number(drive.acceleration)||0,
    frontLateral:Number(grip.frontLateral)||0,rearLateral:Number(grip.rearLateral)||0,
    ...
    frontForceScale:Number(grip.frontLateralForceScale)||0,rearForceScale:Number(grip.rearLateralForceScale)||0,
...
for(const vehicleId of VEHICLE_IDS){
  for(const pitchDeg of PITCHES){
    for(const twist of TWISTS){
      ...
      for(const speed of SPEEDS)for(const steerInput of STEER_INPUTS)for(const throttle of THROTTLES){
        cases.push({vehicleId,pitchDeg,twist,speedMps:speed,speedKmh:speed*3.6,steerInput,throttle,touching,minFactor,meanFactor,factors,...gripProbe(support.vehicle,support.contacts,{speed,steerInput,throttle})});
      }
    }
  }
}

assert.ok(cases.every(entry=>Object.values(entry).every(value=>Array.isArray(value)?value.every(Number.isFinite):(typeof value!=='number'||Number.isFinite(value)))),'Issue #13 diagnostic produced non-finite physics values');
...
const isBreakaway=entry=>entry.frontLateral>=.15&&entry.rearLateral>=.15;
const lowSpeedBreakaway=slopedTransitions.filter(isBreakaway);
...
const bySpeed=SPEEDS.map(speed=>{
  const sample=slopedTransitions.filter(entry=>entry.speedMps===speed);
  const breaks=sample.filter(isBreakaway);
  const moderateBreaks=breaks.filter(entry=>entry.steerInput<=.65);
  return {
    speedMps:speed,speedKmh:speed*3.6,cases:sample.length,
    breakawayCases:breaks.length,moderateSteerBreakawayCases:moderateBreaks.length,
    worstFourWheel:Number(Math.max(0,...sample.map(entry=>entry.fourWheelLateral)).toFixed(3))
  };
});
const bySteer=STEER_INPUTS.map(steerInput=>{
  const sample=slopedTransitions.filter(entry=>entry.steerInput===steerInput);
  return {steerInput,cases:sample.length,breakawayCases:sample.filter(isBreakaway).length};
});
const byThrottle=THROTTLES.map(throttle=>{
  const sample=slopedTransitions.filter(entry=>entry.throttle===throttle);
  return {throttle,cases:sample.length,breakawayCases:sample.filter(isBreakaway).length};
});
const veryLow=slopedTransitions.filter(entry=>entry.speedMps<=3);
const veryLowBreaks=veryLow.filter(isBreakaway).sort((a,b)=>b.fourWheelLateral-a.fourWheelLateral);
const moderateVeryLowBreaks=veryLowBreaks.filter(entry=>entry.steerInput<=.65);

console.log('ISSUE 13 LOW-SPEED LATERAL SLIDE DIAGNOSTIC',JSON.stringify({
  vehicles:VEHICLE_IDS.length,supportCases:supportSummaries.length,physicsCases:cases.length,
  degradedSupportCases:degradedSupport.length,lowSpeedFourWheelBreakawayCases:lowSpeedBreakaway.length,
  veryLowSpeedCases:veryLow.length,veryLowSpeedBreakawayCases:veryLowBreaks.length,
  moderateSteerVeryLowBreakawayCases:moderateVeryLowBreaks.length,
  bySpeed,bySteer,byThrottle,
  flatWorst:[...flatControls].sort((a,b)=>b.fourWheelLateral-a.fourWheelLateral)[0]||null,
  worstSupport:sortedSupport[0]||null,worstFourWheel:sortedFourWheel[0]||null,
  topVeryLowBreakaway:veryLowBreaks.slice(0,12),
  topModerateVeryLowBreakaway:moderateVeryLowBreaks.slice(0,12),
