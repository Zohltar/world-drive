...
import {R15_PROFILE,renderedWindowOptions,r12ContextEligible} from '../tools/biomes/rendered-pilot-policy-r12.mjs';
const [pilot,output]=process.argv.slice(2);
if(!pilot)throw new Error('Use: node qa-block8-rendered-yungas-data-r15.mjs PILOT_DIR [REPORT]');
const data=path.resolve(pilot,'public/local-data/biomes/pilot-r15');
...
assert.equal(tiles.size,15);let windows=0;
for(const direction of [1,-1])for(let i=0;i<oracle.coordinates.length;i++){
  const w=plan.window(plan.distanceAtVertex(i),renderedWindowOptions(R15_PROFILE,direction));
...
const baseUrl='http://127.0.0.1/r15-data/';
const fetchImpl=async url=>{
  const u=new URL(url);assert.equal(u.origin,'http://127.0.0.1');assert.ok(u.pathname.startsWith('/r15-data/'));
  const name=u.pathname.slice('/r15-data/'.length);assert.match(name,/^(?:batch-\d+-\d+\.json|\d+-\d+\.json\.gz)$/);
...
const session=createBiomeBatchRouteSession({source,transport}),adapter=createForestCandidateAdapter({origin:oracle.origin,routeId:'r15-data'});
...
try{
  ...
  for(const sample of oracle.samples){
    assert.equal((await session.update(plan.distanceAtVertex(sample.anchorIndex),renderedWindowOptions(R15_PROFILE))).status,'ready');
    ...
    for(let i=0;i<1744;i++){
      ...
      assert.equal(r12ContextEligible(c,R15_PROFILE),id===444);all&&=id===444;ids.add(id);
      regions[id]=(regions[id]??0)+1;reads++;
    }
    if(all)eligible++;else if(ids.size>1)mixed++;else other++;
  }
  assert.deepEqual({eligible,mixed,other},{eligible:265,mixed:0,other:0});
...
const report={status:'PASS',coverageWindows:windows,sourceChunks:oracle.samples.length,reads,eligible,mixed,other,regions,
  boundaries:'Mixed and non-444 chunks must retain original geometry, not a guessed uniform tropical tree',diagnostics,
