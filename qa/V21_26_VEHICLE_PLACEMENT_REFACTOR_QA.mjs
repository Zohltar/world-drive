...
assert.ok(carSet,'placeAt did not position car render root');
assert.ok(Math.abs(carSet.x-9)<1e-12,`placeAt car render X changed: ${carSet.x}`);
assert.ok(Math.abs(carSet.y-5.5)<1e-12,`placeAt car render Y changed: ${carSet.y}`);
assert.ok(Math.abs(carSet.z-18)<1e-12,`placeAt car render Z changed: ${carSet.z}`);
