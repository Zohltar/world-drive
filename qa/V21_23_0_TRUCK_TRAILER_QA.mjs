...
// Current low-speed tractor geometry preserves the V21.23.2+ hairpin contract:
// about 50.6 deg of mechanical wheel cut at a stop. The long-wheelbase tractor
// still turns wider than the WRX, while the parking-only boost avoids the old
// 6.5–9.5 m radius that made tight mountain hairpins impractical.
...
assert.ok(truckRadius>4.3&&truckRadius<4.7,`tractor parking turn radius ${truckRadius.toFixed(2)} m`);
assert.ok(truckRadius>wrxRadius*1.10,'tractor must still turn materially wider than WRX');
