...
assert.equal(pkg.version,'21.23.3');
assert.ok(main.includes("version:'21.23.3-candidate'"));

// Core terrain/satellite/dynamics/presentation remain byte-for-byte baseline.
const unchanged={
...
for(const needle of [
  ...
  'chunkSegments:96'
]) assert.ok(main.includes(needle),`V21.22.6 policy missing: ${needle}`);

console.log('V21.23.3 BASELINE REGRESSION QA: PASS');
