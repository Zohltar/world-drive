...
for(const pattern of [
  ...
  /Math\.tan\(roadFrame\.pitch\|\|0\)/,
  /Math\.tan\(roadFrame\.roll\|\|0\)/,
  ...
  /Math\.abs\(rs\.lateral\)<roadHalfWidth/,
...
assert.equal(terrainCalls,0,'fast local road plane unexpectedly performed a terrain lookup');

const roadHeight=controller.groundHeightForWheel(50,0,false);
assert.equal(roadHeight,42,'road-surface wheel support changed');
assert.equal(roadCalls,1,'road fallback lookup count changed');
assert.equal(terrainCalls,0,'road-supported wheel unexpectedly fell back to terrain');
...
assert.equal(terrainCalls,1,'terrain fallback lookup count changed');
