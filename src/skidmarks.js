...
export function createSkidMarkSystem({THREE,scene,getWorldOffset,getRoadSurface,maxSegments=7200}){
  ...
  // V21.31 stress fix: roadSurfaceAt() returns the engineered profile surface,
  // while the visible asphalt mesh is built 10 cm above that profile. Skid marks
  // must therefore sit on the asphalt top, not only a few millimetres above the
  // profile where they can disappear inside the road volume.
  const ROAD_ASPHALT_TOP=.10,SKID_SURFACE_CLEARANCE=.004;
