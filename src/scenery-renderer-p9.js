...
export function createSceneryRenderer({
  ...
  function clear(){
    ...
    // Their position is corrected by the streamed forest parent and only newly
    // visible chunks are generated by the P9 streamer.
    refreshForestMasks();
  }
  ...
  function rebuild(){
    ...
    const masksChanged=refreshForestMasks();
    const offset=getWorldOffset();
    ...
    // If OSM exclusion masks changed or the visible terrain was rebuilt, only
    // the nearby forest chunks are regenerated. The outer forest and its cache
    // remain intact, avoiding the P7/P8 multi-thousand-tree rebuild.
    if(masksChanged)forestStreamer.refreshVisibleHeights();
  ...
  loadForestWaterAssets().then(asset=>{
    forestAssets=asset;
    forestStreamer.setAssets(asset);
  }).catch(error=>{
