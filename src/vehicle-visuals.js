...
export function createVehicleVisualSystem({
  ...
  // These materials remain part of the public contract because multiplayer
  // visuals and shared brake-light state still consume them.
  ...
  function updateBrakeLights(dt,braking){
    ...
    tailMat.color
      .copy(brakeBaseColor)
      .lerp(brakeHotColor,brakeLightLevel);
    ...
    for(const entry of extraBrakeLampMaterials){
      entry.material.color
        ...
        .lerp(entry.hotColor,brakeLightLevel);
    }
  }

  // -------------------------------------------------------------------------
  // Invisible wheel probes
  // -------------------------------------------------------------------------
  // Vehicle presentation/suspension still needs real wheel pivot objects and
  // tire geometry metadata. Keep the proven V21.24.94 locations/radii exactly,
  // but never render these probe meshes. Authored GLB modules own all visible
  // tires, rims and body geometry.
  const wheels=[];
  const probeMaterial=new THREE.MeshBasicMaterial({color:0x000000});

  function addWheelProbe({
    vehicleId,
    x,
    y=0,
    z,
    ...
    // Keep tire/rim objects because existing presentation code reads tire
    // geometry width and some animation paths still expect these properties.
    ...
    const rim=new THREE.Mesh(
      new THREE.CylinderGeometry(radius*.60,radius*.60,width+.01,8),
      probeMaterial
    );
  ...
  function addFourWheelProbes({vehicleId,x,z,radius,width=.27,y=0}){
    // Preserve historical order: rear-left, front-left, rear-right, front-right.
  ...
  // ID.4 procedural reference wheel locations.
  addFourWheelProbes({
    vehicleId:'id4',
    x:.86,
    z:1.22,
  ...
  // WRX procedural reference wheel locations.
  addFourWheelProbes({
    vehicleId:'wrx',
    x:.86,
    z:1.25,
  ...
  // Civic legacy buildRoadCarVisual geometry.
  addFourWheelProbes({
    vehicleId:'civic',
    x:1.80*.47,
    z:2.70*.5,
  ...
  // Sonata legacy buildRoadCarVisual geometry.
  addFourWheelProbes({
    vehicleId:'sonata',
    x:1.86*.47,
    z:2.80*.5,
  ...
  // BMW i3 legacy buildRoadCarVisual geometry.
  addFourWheelProbes({
    vehicleId:'i3_2017',
    x:1.78*.47,
    z:2.57*.5,
  ...
  // F1 2010 procedural reference wheel locations.
  addFourWheelProbes({
    vehicleId:'f1_2010',
    x:.88,
    z:1.48,
  ...
  // Countach keeps its staggered front/rear geometry.
  for(const side of [-1,1]){
    addWheelProbe({
      vehicleId:'countach_80',
      x:.91*side,
      z:-1.225,
      radius:.38,
      width:.34,
  ...
  function applyVehicleVisualProfile(){
    ...
    // Probe pivots remain selectable for suspension/physics but their child
    // meshes are permanently invisible. GLB systems may additionally hide the
    // pivots while they own the visible vehicle; restoring them is harmless.
    for(const wheel of wheels){
      wheel.pivot.visible=wheel.vehicleId===id;
    }

    // Preserve V21.24.94 sprung-body stance values. GLB hosts live under this
    // group and therefore retain the exact same vertical calibration.
    const stance={
      id4:-.22,
      wrx:-.31,
      civic:-.30,
    ...
    bodyGroup.position.y=stance[id]??-.28;
  }

  applyVehicleVisualProfile();

  // -------------------------------------------------------------------------
  // Shared automatic headlights
  // -------------------------------------------------------------------------
  // This rig is intentionally retained in V21.25 because gameplay currently
  // uses it for projected road illumination independently of GLB emissive lamps.
  ...
  function updateAutomaticHeadlights(daylight){
    ...
    for(const light of headlightLights){
      light.intensity=185*headlightLevel;
    }
  ...
  // Preserve the historical car-root scale used by suspension/contact math and
  // all existing GLB calibration.
  car.scale.set(.80,.80,.80);
  scene.add(car);

  function activeVehicleWheels(){
    return wheels.filter(
      wheel=>wheel.vehicleId===vehicleSystem.activeId
    );
  }
