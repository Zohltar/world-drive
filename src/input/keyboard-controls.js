...
export function createKeyboardControls({
  ...
  function codes(action){
    ...
    if(action==='clutch'&&(!Array.isArray(configured)||!configured.length)&&(!Array.isArray(fallback)||!fallback.length))return ['ShiftLeft'];
    return Array.isArray(configured)&&configured.length?configured:fallback;
  }

  function actionDown(action){return codes(action).some(code=>!!keys[code]);}
  function actionMatches(action,code){return codes(action).includes(code);}
  function clearState(){for(const key of Object.keys(keys))delete keys[key];}
  function assignBinding(action,code){
    ...
    controls[action]=[code];queueSettingsSave?.();
  }
  ...
  function keydown(e){
    ...
    if(rebindAction){
      ...
      setKeyboardRebindAction(null);assignBinding(rebindAction,e.code);
    ...
    if(!state.gameStarted||state.menuOpen||inputTag==='INPUT'||inputTag==='TEXTAREA'||inputTag==='SELECT'||e.target?.isContentEditable)return;

    keys[e.code]=true;
    if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','BracketLeft','BracketRight','ShiftLeft'].includes(e.code))e.preventDefault();
    ...
    if(state.autopilot&&(actionMatches('steerLeft',e.code)||actionMatches('steerRight',e.code)||actionMatches('brake',e.code)||actionMatches('handbrake',e.code)||actionMatches('clutch',e.code)))onManualTakeover?.();
  }
