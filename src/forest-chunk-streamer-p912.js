...
// Foret P9.12 — transition-safe dense forest streamer.
// One immutable instance matrix buffer is kept per chunk. Density changes only
// adjust mesh.count, so road transitions never re-upload matrices to the GPU.
export function createForestChunkStreamer({
  ...
  const cellsPerBuildSlice=Math.max(1,FOREST.cellsPerBuildSlice||2);
  ...
  function processBuilderCell(builder){
    if(builder.buildSerial!==serial)return false;
    const index=builder.cellIndex++;
    if(index>=chunkCells*chunkCells)return true;
    ...
    const nearRoadCell=cellNearRoad(cellX,cellZ);
    const baseDensity=densityAt(cellX,cellZ);
    ...
    return builder.cellIndex>=chunkCells*chunkCells;
  }
  ...
  function queueJob(desc,{replace=false}={}){
    ...
    const job={...desc,replace,builder:null};
  ...
  function runQueue(){
    ...
    const step=deadline=>{
      ...
      if(!wantedKeys.has(job.key)&&!job.replace){
        ...
        let cells=0;
        while(job.builder.cellIndex<chunkCells*chunkCells&&cells<cellsPerBuildSlice){
          if(cells>0&&!deadline.didTimeout&&deadline.timeRemaining()<2.5)break;
          processBuilderCell(job.builder);
          cells++;
        }
        if(job.builder.cellIndex>=chunkCells*chunkCells){
  ...
  function refreshVisibleHeights(){
    serial++;
    for(const job of queue)job.builder=null;
