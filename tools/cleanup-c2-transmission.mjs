...
base=replaceExact(
  ...
  'explicit combustion Neutral branch'
);
base=replaceExact(
  base,
  '    if(getSpeed()<-.25){',
  '    if(selector<0){',
  'explicit Reverse selector branch'
);

// Canonical wrapper: remove cross-file layer and distinguish selector writes
// from publication. Publication after update is observational only; it no
// longer repairs a gear value that the hidden core wrote incorrectly.
...
// The post-update call is the only place where selector-state application must
// become publication-only. Replace the exact call following core update.
wrapper=replaceExact(
  wrapper,
  '      let transmitted=coreUpdateTransmission(dt,baseRequested,onPavement,automaticOverride);\n      applySelectorState();',
  '      let transmitted=coreUpdateTransmission(dt,baseRequested,onPavement,automaticOverride);\n      publishAuthoritativeGear();',
  'post-update publication only'
);
...
wrapper=replaceExact(
  ...
  'public forward gear normalization'
);
...
// Keep imports first, then private gearbox implementation, then the public
// selector/free-rev/controller layer. This remains one module and one owner.
...
// V21.26 QA predates the exact D/N/R protocol. Modernize only its stale EV
// expectation; its source-location checks become valid again after consolidation.
let staleQa=fs.readFileSync(staleQaPath,'utf8');
staleQa=replaceExact(
  staleQa,
  ...
  'modernize stale V21.26 EV gear expectation'
);
fs.writeFileSync(staleQaPath,staleQa);

console.log('C2 transmission consolidation materialized',{
  removed:basePath,
  canonical:controllerPath,
  selectorContract:{reverse:-1,neutral:0,forward:'1..N'}
});
