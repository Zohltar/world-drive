...
export function createMixedForestPresentation({THREE,group,source,kit,proof,season,now=()=>performance.now()}){
  ...
  function asset(family,value){const id=seasonalVegetationId(R16_MODELS[family],value);
  ...
  try{
    for(const family of [0,1]){
      ...
      const a=asset(family,season),mesh=new THREE.InstancedMesh(a.geometry,material,map.indices[family].length);
      ...
      const part={family,id:a.id,mesh,indices:map.indices[family]};parts.push(part);
  ...
  function setSeason(value){
    ...
    parts.forEach((p,i)=>{p.id=assets[i].id;p.mesh.geometry=assets[i].geometry;});currentSeason=value;return true;
  }
  function diagnostics(){
    ...
    return {models,instances:count,parts:parts.length,potentialAdditionalDrawCalls:Math.max(0,Object.values(models).filter(n=>n>0).length-1),
      accountedBytes:parts.reduce((n,p)=>n+p.mesh.instanceMatrix.array.byteLength+p.indices.byteLength,0)
        +map.prefix.reduce((n,p)=>n+p.byteLength,0),builds,syncs,callbacks,maxSyncMs,failed,disposed,season:currentSeason};
  }
  function audit(){
    ...
    const visible=parts.map(p=>{
      ...
      return {model:p.id,count,matrixHash:hash(a.subarray(0,count*16)),triangles:(p.mesh.geometry.index?.count??p.mesh.geometry.attributes.position.count)/3};
    });
