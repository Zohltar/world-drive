#!/usr/bin/env python3
"""Explicit build-time capture of the game's real Manic-2 -> Manic-5 route.
...
def validate(raw):
    ...
    if geometry.get('type') != 'LineString' or not isinstance(pts, list) or not 100 <= len(pts) <= 20000:
        raise ValueError('Expected a full road polyline of 100..20000 vertices')
    for p in pts:
        if (not isinstance(p, list) or len(p) != 2
            or any(type(v) not in (int, float) or not math.isfinite(v) for v in p)
            or not (-70 <= p[0] <= -67 and 49 <= p[1] <= 51)):
            raise ValueError('Invalid or out-of-region Manic position')
    if any(distance(p, expected) > 2000 for p, expected in zip((pts[0], pts[-1]), POINTS)):
        raise ValueError('Endpoint mismatch or reversed route')
    lengths = [distance(a, b) for a, b in zip(pts, pts[1:])]
    total = sum(lengths)
    if not 100000 <= total <= 400000 or max(lengths) > 5000:
...
def capture(output):
    ...
    for prefix in URLS:
        ...
        try:
            ...
            receipt = {'schema':'world-drive-real-road-capture-v1', 'route':'manic2-manic5',
